let matchesCounter = document.querySelector('.all-matches');
...
const render = () => {
    matchesCounter.innerHTML = '';
    const state = store.getState();
    state.map((data) => {
        matchesCounter.innerHTML += `
        <div class="match">
            <div class="wrapper">
                <button class="lws-delete" onClick="handleDelete.call(this, event)">
                    <img data-id=${data.id} src="./image/delete.svg" alt="" />
                </button>
                <h3 class="lws-matchName">${data.match}</h3>
            </div>
            <div class="inc-dec">
                <form class="incrementForm" data-id=${data.id} onSubmit="handleIncrement.call(this, event)">
                    <h4>Increment</h4>
                    <input type="number" name="increment" class="lws-increment" />
                </form>
                <form class="decrementForm" data-id=${data.id} onSubmit="handleDecrement.call(this, event)">
                    <h4>Decrement</h4>
